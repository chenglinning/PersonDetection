...
void write_BoxData(image im, int num, float thresh, box *boxes, float **probs, char **names, image **alphabet, int classes,char *outfile)
{
    int i;
    int people_count=0;
    char DataFileName[256];
    sprintf(DataFileName,"%s.csv",outfile);
    FILE *fp=fopen(DataFileName,"w+");
    for(i = 0; i < num; ++i){
        int class = max_index(probs[i], classes);
        float prob = probs[i][class];
        if(prob > thresh && 0 == strcmp(names[class], "person")){
            int width = im.h * .012;
            people_count++;
            printf("%s%d: %.0f%%\n", names[class],people_count, prob*100);
            if(0){
            ...
            //printf("%s%d:x=%d y=%d w=%d h=%d\n",names[class],people_count,left,top,right-left,bot-top);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        write_BoxData(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes,outfile);
        /*
        if(outfile){
